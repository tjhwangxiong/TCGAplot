#' gene_TMB_radar
#'
#' @description Create a pan-cancer radar chart for gene expression and TMB correlation.
...
#' @export
#' @examples gene_TMB_radar("KLF7")
gene_TMB_radar=function(gene,method = "pearson"){
  data("tpm")
  ...
  for (cancer in cancers) {
    ...
    tmp <- cor.test(df1[gene,], dat1$TMB,method = "pearson")
  ...
  fmsb::radarchart(res,
             ...
             # Customize the axis
             axislabcol = "black",
             title=paste0("Correlation between ",gene, " and TMB"),
             # Variable labels
