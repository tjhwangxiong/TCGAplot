...
gs_paired_boxplot=function(cancer,geneset,palette="jco",legend="none",label="p.signif",method="wilcox.test"){
  ...
  if (cancer %in% pcancers) {
    ...
    p + stat_compare_means(label = label,method = method,paired=T,label.x.npc='center',label.y.npc ='top')
  }
  else
