#' methy_kmplot
#'
#' @description Describes the K_M survival plot based on the promoter methylation of a single gene in a specific type of cancer. A pdf file named methylation_kmplot will be generated in the working directory.
...
methy_kmplot=function(cancer,gene,palette='jco'){
  print('A pdf file named methylation_kmplot will be generated in the working directory.')
  data("cg_probe","methy")
  cg_list=cg_probe$probe[cg_probe$gene==gene]

  exprSet=dplyr::select(methy,all_of(cg_list))%>%
  ...
  # plot
  pdf("methylation_kmplot.pdf",width = 8,height = 6)
  for (cg in cg_list){
    cl1=cl%>%
