#' paired_boxplot
#' @description Create a paired tumor-normal box plot for a single gene with symbols indicating statistical significance in a specific type of cancer. Only cancers with more than 20 paired samples could be analyzed, including "BLCA","BRCA","COAD","ESCA","HNSC","KICH","KIRC","KIRP","LIHC","LUAD","LUSC","PRAD","STAD","THCA", and "UCEC".
...
paired_boxplot=function(cancer,gene,palette="jco",legend="none"){
  ...
  if (cancer %in% pcancers) {
    ...
    p=ggpaired(df,x="Group", y=gene,
               ...
               xlab='',ylab=paste0(gene,' in ',cancer),
    ...
    p + stat_compare_means(label="p.signif",paired=T,label.x.npc='center',label.y.npc ='top')
  }
  else
